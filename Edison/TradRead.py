...
def openSerial():
	...
	PORT = '/dev/{}'.format(min(output)) # This one is the min of the coms, the other will be max
	if debug: print PORT 
...
def writeToFile(x):
	...
	output_jsons.append({'source': 'realTesting', 'test': 'lowerCOM', 'DOF': '1DOF', 'packet': x})
	try:	
		resultFile = open("trad/data_lcom_{}.json".format(epoch_time),'wb')
